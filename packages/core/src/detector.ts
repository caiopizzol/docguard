...
import { DetectionResult, Term, Context, PatternSet, FileChange, DetectorOptions } from './types';
...
export class Detector {
    ...
    private detectInFile(change: FileChange): DetectionResult[] {
        ...
        for (const [category, patterns] of Object.entries(this.patterns)) {
            for (const pattern of patterns) {
                const result = this.detectPattern(change.path, category, pattern, beforeLines, afterLines);
                if (result && (result.removals.length > 0 || result.additions.length > 0)) {
                    results.push(result);
                }
            }
        }
    ...
    private findTermsInLines(lines: string[], pattern: any): Term[] {
        ...
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            if (pattern.regex) {
                const regexMatches = this.findRegexInLine(line, pattern.regex, lineIndex);
     ...
     */
    private findTermInLine(line: string, term: string, lineNumber: number): Term[] {
        const terms: Term[] = [];
     ...
     */
    private findRegexInLine(line: string, regex: RegExp, lineNumber: number): Term[] {
        const terms: Term[] = [];
