...
export class GitDiffer {
     ...
     */
    async getChangedFiles(baseCommit: string = 'HEAD', targetCommit?: string): Promise<FileChange[]> {
        if (!this.isGitRepository()) {
            throw new Error('Not a git repository. DocGuard needs a git repository to compare changes.');
        }
        ...
        try {
            ...
            for (const file of changedFiles) {
                if (this.isDocumentationFile(file)) {
                    const change = await this.getFileChange(file, baseCommit, targetCommit);
                    if (change) {
                        changes.push(change);
                    }
                }
     ...
     */
    private async getFileChange(file: string, baseCommit: string, targetCommit?: string): Promise<FileChange | null> {
