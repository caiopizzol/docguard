...
export class TerminalFormatter {
    format(results: DetectionResult[]): string {
        ...
        for (const result of results) {
            ...
            output.push(`✗ ${this.formatCategory(result.category)} removed from ${result.file}:${result.removals[0]?.line || '?'}`);
        ...
        output.push('');
        output.push(`This change removed critical terms. Verify this is intentional.`);
